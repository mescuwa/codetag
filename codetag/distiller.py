...
def distill_level_2(content: str, *, include_anchors: bool = True) -> str:
    ...
    try:
        ...
        distilled = _StructureDistiller(content.splitlines(), include_anchors=include_anchors).visit(tree)
...
def distill_repository(
    *,
    ...
    for fp in all_files:
        ...
        if fp.is_symlink() or fp.resolve() in {output_file.resolve(), map_file.resolve()}:
            continue
        ...
        mappings.append(
            {
                "distilled_range": [current_line + 3, current_line + 3 + content_lines - 1],
