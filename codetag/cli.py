...
# Placeholder removed – real TUI lives in codetag.tui.run_tui
...
app = typer.Typer(
    ...
    help=(
        ...
        "Made by Lumina Mescuwa (@mescuwa) — https://github.com/mescuwa"
    ),
...
def scan_repository(
    ...
    path: Path = typer.Argument(
        ..., exists=True, resolve_path=True, help="Directory to analyze."
    ),
    ...
    no_cache: bool = typer.Option(
        False, "--no-cache", help="Disable on-disk caching for faster re-scans."
    ),
    exclude_dirs: Optional[str] = typer.Option(
        None, "--exclude-dirs", help="Comma-separated list of directories to exclude."
    ),
    ...
    if report.threat_assessment.secrets_found:
        typer.echo(
            f"🚨 WARNING: Found {len(report.threat_assessment.secrets_found)} potential secret(s).",
            err=True,
...
def pack(
    ...
    exclude_extensions: str = typer.Option(
        "", help="Comma-separated list of additional file extensions to exclude."
    ),
    max_tokens: int = typer.Option(
        250_000,
        "--max-tokens",
        "-t",
        help="The maximum total number of tokens for the packed output.",
    ),
    format: str = typer.Option(
        "raw", "--format", help="Output format: 'raw' or 'json'."
    ),
):
    ...
    try:
        ...
        typer.echo(
            f"✅ Packed {stats['files_selected']} files (~{stats['final_token_count']:,} tokens) into {output_file} in {stats['duration_seconds']}s.",
            err=True,
...
def distill(
    path: Path = typer.Argument(
        ..., exists=True, resolve_path=True, help="Directory to distill."
    ),
    output_file: Path = typer.Option(
        "distilled.txt",
        "-o",
        "--output-file",
        help="Path to save the distilled context file.",
    ),
    level: int = typer.Option(
        ...
        min=1,
        max=2,
        help="Distillation level (1: cleanup, 2: structural).",
    ),
    anchors: bool = typer.Option(
        True,
        "--anchors/--no-anchors",
        help="Include SHA-256 anchors for stripped Python function bodies.",
    ),
):
    """Distill a codebase into a concise, LLM-friendly summary."""

    typer.echo(f"💧 Distilling codebase at: {path} (Level {level})", err=True)
    try:
        ...
        typer.echo(
            f"✅ Distilled {stats['files_processed']} files to ~{stats['total_tokens']:,} tokens into {output_file} in {stats['duration_seconds']}s.",
            err=True,
        )
        typer.echo(f"🗺️  Source map saved to {stats['map_file_path']}", err=True)
    except Exception as exc:
        typer.echo(f"❌ Error during distillation: {exc}", err=True)
        raise typer.Exit(code=1)
...
def audit(
    ...
    strict: bool = typer.Option(
        False, "--strict", help="Use a stricter community ruleset for Semgrep."
    ),
):
    ...
    if assessment.dependency_vulnerabilities:
        ...
        for v in assessment.dependency_vulnerabilities:
            tbl.add_row(
                v.id,
                v.package_name,
                v.vulnerable_version,
                v.severity or "?",
                v.summary[:60] + ("…" if len(v.summary) > 60 else ""),
            )
        console.print(tbl)
    ...
    if has_findings:
        typer.secho(
            "Audit finished: Found potential issues.", fg=typer.colors.YELLOW, err=True
        )
        raise typer.Exit(code=1)
    else:
        typer.secho(
            "✅ Audit finished: No issues found.", fg=typer.colors.GREEN, err=True
        )
